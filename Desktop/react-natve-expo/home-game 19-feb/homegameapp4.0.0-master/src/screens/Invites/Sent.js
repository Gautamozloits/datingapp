...
export default class Sent extends React.Component {
        ...
        setTimeout(() => {
            Alert.alert('Home Game', 'Are you sure want to delete this game?', [{
                text: 'Yes', onPress: async () => {
                    ...
                    postData('schedule-game/delete', params).then(async (res) => {
                        if (res.status) {
    ...
    dateBoxCard = (item, date_type) => {
        ...
        return (
            item[date_type] ?
                ...
                //onPress={() => this.selecteDate(item, date_type, item[date_key])}
                >
                ...
                <View style={[Styles.tbRow]}>
                    ...
                    <View style={[Styles.tbCol, { width: '10%', justifyContent: 'flex-start', alignItems: 'center', }]}>
                        ...
                        <ActionDropdown
                            ...
                            color='#A8B0BC'
                            options={(item.is_poll_open === 1) ? ['Close Poll', 'Delete'] : ['Open Poll', 'Delete']}
                            dropdownStyle={Styles.dropdown_2_dropdown}
                            onSelect={(idx, value) => {
                                if(value == 'Delete'){
                                    this.deleteGame(item.game_id, index)
                                }else{
                                    this.updatePollStatus(item, index, value)
                                }
                            }}
                        />
                    </View>
    ...
    renderView = () => {
        ...
        } else if (currentSubView === 'EditTable') {
            return (<UsersEditTableVIew that={this} />)
        } else {
            return (
                <>
                    ...
                    <AddNewPersonModalGuest isVisible={this.state.isAddPlayerModalVisible} that={this} /> */}
