...
export function PhoneBooks(props) {
    ...
    (async () => {
      ...
      if (status === 'granted') {
        ...
        if (props.isVisible) {
          ...
          props.that.setState({ isPhoneBookModalVisible: true })
        }
        getAllData();
      }else{
        if(Platform.OS == 'ios'){
        ...
        props.that.setState({isAddPlayerModalVisible: true, isPhoneBookModalVisible: false})
  ...
  function closeModal() {
    getAllData();
    props.that.setState({ isPhoneBookModalVisible: false })
  }
  ...
  function getContacts(item, contact) {
    ...
    setLoader(false);
    var number_index = props.that.state.gameDetails.users.findIndex((value) => {
      return value.phone_number === full_number;
    })
    if (number_index >= 0) {
      showLocalMessage('This contact is already added')
    }else{
