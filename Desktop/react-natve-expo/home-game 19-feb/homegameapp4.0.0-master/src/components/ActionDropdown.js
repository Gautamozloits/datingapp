...
export default class ActionDropdown extends Component {
  ...
  _renderButton() {
    const { disabled, accessible, children, textStyle, buttonView, icon, color } = this.props;
          ...
          (
            <View style={styles.button}>
              ...
              >
                ...
                <Entypo
                  name={(icon) ? icon : 'dots-three-vertical'}
                  size={normalize(18)}
                  style={{ marginBottom: -3 }}
                  color={(color) ? color : '#2A395B'}
                />
              </Text>
            </View>
          )
  ...
  _renderModal() {
    const { animated, accessible, dropdownStyle, showModal } = this.props;
    ...
    if (showDropdown && this._buttonFrame) {
      ...
      if (this.props.options.length > 1) {
        height = (42 * this.props.options.length);
      }
