...
class Events extends Component {
  ...
  getEvents = (page = 1) => {
    ...
    postData("all-games", data).then((res) => {
      if (res.success) {
        this.setState({
          notfound: false,
          loading: false,
  ...
  toggleModal = (users = []) => {
    this.setState({ modal: !this.state.modal, game_playsers: users });
  };

  //g code here
  toggleModalTotalBuy = (users = []) => {
    this.setState({
      totalBuyModal: !this.state.totalBuyModal,
      game_playsers: users,
    });
  };

  userModal() {
    const { modal, game_playsers } = this.state;
    return (
      <Modal isOpen={modal}>
        <ModalHeader>Game Players</ModalHeader>
        <ModalBody style={{ maxHeight: "300px", overflow: "auto" }}>
          <ListGroup>
            {game_playsers.map((user, index) => {
              return (
                <ListGroupItem
                ...
                >
                  {user.image ? (
                    <img
                      className="img-fluid"
                      src={Global.UPLOAD_URL + user.image}
                      alt=""
                      height="50px"
                      width="50px"
                    />
  ...
  toggleCollapse(user, index) {
    // console.log("consoling user index = ", user, index)
    const { game_playsers } = this.state;
    // console.log("consoling    game_playsers = ", game_playsers)
    if (!user.is_collapse || user.is_collapse == "") {
      user.is_collapse = "show";
    } else {
      user.is_collapse = "";
    }
    game_playsers[index] = user;
    // console.log("consoling    game_playsers[index] = ", game_playsers[index])
    this.setState({ game_playsers: game_playsers });
    // console.log("consoling    game_playsers = ", game_playsers)
  }
  userModalTotalBuy() {
    const { totalBuyModal, game_playsers } = this.state;

    return (
      <Modal isOpen={totalBuyModal}>
        <ModalHeader>Game Total Buy In</ModalHeader>
        <ModalBody style={{ maxHeight: "300px", overflow: "auto" }}>
          {game_playsers.map((user, index) => {
            console.log("json printing", game_playsers);

            return (
              <div id={"accordion" + user.id}>
                <div className="card"  >
                  <div className="card-header" style={{padding:"0px"}}  id="headingOne">
                    <h5 className="mb-0">
                      <button
                        className="btn btn-block" 
                        onClick={() => this.toggleCollapse(user, index)}
                        style={{align:"center", decoration:"none", width:"100%"}}>
                  ...
                  <div className={"collapse " + user.is_collapse}>
                    <div className="card-body">
                      <ListGroup>
                        {user.buyins.map((userTwo, index) => {
                          console.log(
                            "user printing",
                            userTwo.buyin,
                            userTwo.created_at,
                          ...
                          return (
                            <ListGroupItem
                              key={
                                userTwo.id.toString() + Math.random().toString()
                              }
                            >
        ...
        <ModalFooter>
          <Button color="secondary" onClick={() => this.toggleModalTotalBuy()}>
            OK
          </Button>
        </ModalFooter>
      </Modal>
